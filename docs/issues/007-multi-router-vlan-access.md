...
Devices connected to the **office router** (same L2 segment as Proxmox/OPNSense) can reach PROD VLAN services (10.10.10.0/16) via a static route through OPNSense WAN (192.168.1.245). However, devices connected to the **home (room) router** cannot, even with the same static route configured on the device.
...
Home Router (192.168.1.1) ← main router, DHCP server
 ...
 │    ├── daytona (192.168.1.5)
 │    ├── OPNSense WAN (192.168.1.245, DHCP)
...
When the laptop is on the **office router**, it shares L2 with OPNSense WAN. The static route (`10.10.0.0/16 via 192.168.1.245`) works because:
...
1. Laptop sends packet toward 192.168.1.245, but it goes through the **home router first**
...
Configure the home router to use OPNSense (192.168.1.245) as its default gateway, or set up OPNSense as the DHCP server pushing itself as the gateway.
...
- [ ] Any device on 192.168.1.0/24 (regardless of which router/AP) can ping 10.10.10.10
