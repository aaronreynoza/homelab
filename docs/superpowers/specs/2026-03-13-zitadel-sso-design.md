...
Redirect URIs use Cilium LB-IPAM IPs (from the 10.10.10.220-250 pool). Actual IPs are populated in `terraform.tfvars` from `kubectl get svc` output. All flows are HTTP (Zitadel runs with `ExternalSecure: false`).
