...
| Management IP | 192.168.1.10 (static, untagged) |
| VLAN 10 IP | 10.10.10.5 (static, tagged) |
...
- Forgejo: deployed at 10.10.10.222:3000, running, healthy, but empty

### Target State

- Source of truth: Forgejo at 10.10.10.222:3000
...
server:
  DOMAIN: 10.10.10.222
  ROOT_URL: http://10.10.10.222:3000
```
...
/kaniko/executor \
  ...
  --destination=10.10.10.223/platform/docs:${COMMIT_SHA} \
  --destination=10.10.10.223/platform/docs:latest \
  --insecure \
  --cache=true \
  --cache-repo=10.10.10.223/cache/docs \
  --snapshot-mode=redo
```
      ...
      - name: Build and push docs image
        run: |
          /kaniko/executor \
            ...
            --dockerfile=Dockerfile.docs \
            --destination=10.10.10.223/platform/docs:${{ github.sha }} \
            --destination=10.10.10.223/platform/docs:latest \
            --insecure \
            --cache=true \
            --cache-repo=10.10.10.223/cache/docs
