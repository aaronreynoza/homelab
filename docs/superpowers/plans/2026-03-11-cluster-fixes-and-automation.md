...
**Note:** `k8sServiceHost` is hardcoded to `10.10.10.10` (the prod control plane IP). For a reusable config across environments, this could be templated. For now it matches the prod deployment.
...
- Static route on Mac (`sudo route add -net 10.10.0.0/16 192.168.1.245`)
