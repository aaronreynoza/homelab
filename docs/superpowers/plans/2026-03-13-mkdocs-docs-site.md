...
**Architecture:** Multi-stage Docker build (MkDocs Material generates static HTML, Caddy serves it). Image pushed to Harbor (REDACTED_LB_IP), deployed via ArgoCD as plain K8s manifests in the `docs` namespace, exposed through Pangolin.
...
Harbor is at `REDACTED_LB_IP`. Since Harbor is using HTTP (not HTTPS) internally, the K8s nodes need Harbor's IP in their container runtime's insecure registries list. If this isn't already configured, the image pull will fail with a TLS error. Check by examining the Talos machine config or by attempting to pull.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: caddy
          image: REDACTED_LB_IP/platform/docs:v1.0.0
          ports:
            - containerPort: 80
          resources:
            requests:
...
**Note:** We use `REDACTED_LB_IP` (Harbor's LB IP) as the registry address rather than `harbor.internal` because there's no internal DNS resolution for that hostname yet (ctrld on OPNSense is not configured). Once DNS is set up, this can be changed to `harbor.internal`.
...
**Context:** LoadBalancer service, same pattern as nginx-test. Cilium LB-IPAM will assign an IP from the REDACTED_LB_IP-250 pool. No need to specify a specific IP — Cilium picks one automatically.
...
**Context:** Harbor is running at `REDACTED_LB_IP`. You need to tag the image for Harbor's registry and push it. Harbor may require login first. The project `platform` may need to be created in Harbor's web UI if it doesn't exist (Harbor at `http://REDACTED_LB_IP`).

- [ ] **Step 1: Create the `platform` project in Harbor (if it doesn't exist)**

Open `http://REDACTED_LB_IP` in a browser. Log in with the admin credentials (from the `harbor-credentials` secret). Go to Projects → New Project → Name: `platform`, Access Level: Public. If the project already exists, skip this step.
...
On macOS (Docker Desktop): Open Docker Desktop → Settings → Docker Engine → add `"insecure-registries": ["REDACTED_LB_IP"]` → Apply & Restart.

On Linux: Add `{"insecure-registries": ["REDACTED_LB_IP"]}` to `/etc/docker/daemon.json` and restart Docker.

- [ ] **Step 3: Log in to Harbor**

Run: `docker login REDACTED_LB_IP`
...
Run: `docker build -f Dockerfile.docs -t REDACTED_LB_IP/platform/docs:v1.0.0 .`

Expected: Build succeeds.

- [ ] **Step 5: Push to Harbor**

Run: `docker push REDACTED_LB_IP/platform/docs:v1.0.0`
...
Expected: Service has an `EXTERNAL-IP` from the REDACTED_LB_IP-250 range.
...
- Name: `docs`
- Value: Pangolin VPS IP (`REDACTED_VPS_IP`) or its hostname
