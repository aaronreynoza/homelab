...
# mount -t nfs 192.168.1.5:/hdd-mirror/media-data /mnt/test
# ls /mnt/test
# umount /mnt/test
```
```
...
**Context:** The media namespace needs `pod-security.kubernetes.io/enforce: privileged` because Gluetun requires `NET_ADMIN` capability and `/dev/net/tun`. The NFS PV points to daytona's management IP (`192.168.1.5`) since K8s pods on VLAN 10 route through OPNSense which has a WAN-to-PROD pass rule. The PVC uses RWX so all media pods can mount it simultaneously.
...
- For LAN access, ensure static route to PROD VLAN: `sudo route add -net 10.10.10.0/16 192.168.1.245`
...
Create Pangolin resources for each user-facing media app. All resources route through the Vultr VPS (207.246.115.3) and terminate at the K8s LoadBalancer IPs assigned by Cilium LB-IPAM.
...
```
watch.home-infra.net    A    207.246.115.3
request.home-infra.net  A    207.246.115.3
music.home-infra.net    A    207.246.115.3
files.home-infra.net    A    207.246.115.3
```
```
