...
## Prerequisites
- Proxmox host (daytona) accessible at 192.168.1.5
...
In the Proxmox web UI (https://192.168.1.5:8006):
...
## 4. Configure Cloud-Init
    ...
    # From workstation:
    #   scp ~/.ssh/id_ed25519.pub root@192.168.1.5:/tmp/mgmt_key.pub

    qm set 99 --ciuser operator
    qm set 99 --sshkeys /tmp/mgmt_key.pub
    qm set 99 --ipconfig0 ip=192.168.1.10/24,gw=192.168.1.1
...
From workstation:
    ...
    ssh operator@192.168.1.10

If using a non-default key:

    ssh -i ~/.ssh/id_ed25519 operator@192.168.1.10
...
### Copy SOPS age key
    scp ~/.config/sops/age/keys.txt operator@192.168.1.10:~/.config/sops/age/keys.txt

### Copy AWS credentials
    scp ~/.aws/credentials operator@192.168.1.10:~/.aws/credentials
...
### Copy kubeconfig and talosconfig
    scp environments/prod/kubeconfig operator@192.168.1.10:~/.kube/config
    scp environments/prod/talosconfig operator@192.168.1.10:~/.talos/config
...
```ini
[mgmt]
192.168.1.10 ansible_user=operator ansible_ssh_private_key_file=~/.ssh/id_ed25519

[mgmt:vars]
ansible_python_interpreter=/usr/bin/python3
mgmt_ip=192.168.1.10
vlan10_ip=10.10.10.5
...
4. Configure cloud-init (user=operator, SSH key, IP=192.168.1.10)
5. Set auto-start order 2
6. Start the VM

- [ ] **Step 2: Verify SSH access**

Run from workstation: `ssh operator@192.168.1.10 'hostname && ip addr show'`

Expected: Hostname `mgmt`, IP `192.168.1.10` on the primary interface.
...
```bash
ssh operator@192.168.1.10 'ping -c 3 10.10.10.1'
```
...
```bash
ssh operator@192.168.1.10 'kubectl version --client && talosctl version --client && terraform version && sops --version'
```
...
```bash
scp ~/.config/sops/age/keys.txt operator@192.168.1.10:/home/operator/.config/sops/age/keys.txt
scp ~/.aws/credentials operator@192.168.1.10:/home/operator/.aws/credentials
```

- [ ] **Step 2: Copy kubeconfig and talosconfig**

```bash
scp environments/prod/kubeconfig operator@192.168.1.10:/home/operator/.kube/config
scp environments/prod/talosconfig operator@192.168.1.10:/home/operator/.talos/config
```
...
talosctl --nodes 10.10.10.10 health
```
...
- Dual-homed: 192.168.1.10 (management) + 10.10.10.5 (VLAN 10)
