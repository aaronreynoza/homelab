...
```bash
ssh admin@192.168.1.10 "curl -s http://<ZITADEL_IP>:8080/debug/ready"
```
...
variable "argocd_url" {
  description = "ArgoCD base URL (e.g., http://10.10.10.221:8080)"
  type        = string
}

variable "forgejo_url" {
  description = "Forgejo base URL (e.g., http://10.10.10.222:3000)"
  type        = string
}

variable "harbor_url" {
  description = "Harbor base URL (e.g., http://10.10.10.223)"
  type        = string
}

variable "grafana_url" {
  description = "Grafana base URL (e.g., http://10.10.10.224:3000)"
  type        = string
}
...
**Context:** Run from the mgmt VM (192.168.1.10) which has Terraform, kubeconfig, and SOPS age key.
