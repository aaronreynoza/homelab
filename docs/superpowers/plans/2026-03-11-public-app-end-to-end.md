...
If no route, add: `sudo route add -net 10.10.0.0/16 192.168.1.245`
...
```bash
talosctl --talosconfig environments/prod/talosconfig --nodes 10.10.10.10 logs ext-newt
```
...
- **Why:** Cilium `k8sServiceHost: "10.10.10.10"` and Pangolin URLs are hardcoded in ArgoCD manifests
...
# Connectivity
talosctl --nodes 10.10.10.10 logs ext-newt           # Newt connected
