...
- Lines 18-22: `global.hostAliases` (hardcoded IP `10.10.10.225` for `zitadel.internal`)
- Line 68: `url: "http://10.10.10.221"` (hardcoded ArgoCD URL)
- Lines 69-77: `oidc.config` block (hardcoded client ID `364000638838833752`, hardcoded issuer)
...
```hcl
variable "zitadel_ip" {
  description = "Zitadel LoadBalancer IP (e.g., 10.10.10.225). Used for ExternalDomain and issuer URL."
  type        = string
}
...
**What this changes:**
- OIDC issuer URL becomes `http://10.10.10.225:8080` (was `http://zitadel.internal:8080`)
- OIDC discovery URL becomes `http://10.10.10.225:8080/.well-known/openid-configuration`
...
curl -s http://10.10.10.225:8080/.well-known/openid-configuration | grep issuer
# Expected: "issuer":"http://10.10.10.225:8080"
```
...
```hcl
# Zitadel
zitadel_url = "http://10.10.10.225:8080"
zitadel_ip  = "10.10.10.225"

# App URLs (from kubectl get svc LoadBalancer IPs)
argocd_url  = "http://10.10.10.221"
forgejo_url = "http://10.10.10.222:3000"
harbor_url  = "http://10.10.10.223"
grafana_url = "http://10.10.10.224:3000"
...
```bash
ssh admin@192.168.1.10 "cd ~/homelab && git pull"
```

- [ ] **Step 3: Copy private files to mgmt VM**

```bash
scp -r environments/prod/zitadel/ admin@192.168.1.10:~/environments/prod/zitadel/
```

- [ ] **Step 4: Terraform init (re-init for new providers)**

```bash
ssh admin@192.168.1.10 << 'REMOTE'
...
# Expected: YAML with name: Zitadel, issuer: http://10.10.10.225:8080, clientID: <actual-id>

kubectl get cm argocd-cm -n argocd -o jsonpath='{.data.url}'
# Expected: http://10.10.10.221
```

- [ ] **Step 2: Test OIDC login**

Open `http://10.10.10.221` in browser. Click "Log in via Zitadel". Authenticate with `aaron@reynoza.org` / `ChangeMe123!` (or whatever was set). Should redirect back to ArgoCD with admin role.
...
Open `http://10.10.10.222:3000` in browser. Click "Sign in with Zitadel". Authenticate. Should create an account linked to your Zitadel identity.
...
Open `http://10.10.10.224:3000` in browser. Click "Sign in with Zitadel". Authenticate. Should land on Grafana dashboard.
...
```bash
curl -s -u "admin:<harbor-password>" http://10.10.10.223/api/v2.0/configurations | python3 -m json.tool | grep oidc
# Expected: auth_mode: oidc_auth, oidc_endpoint: http://10.10.10.225:8080, etc.
```

- [ ] **Step 2: Test OIDC login**

Open `http://10.10.10.223` in browser. Click "Login via OIDC Provider". Authenticate.

- [ ] **Step 3: Verify local admin fallback**

Navigate directly to `http://10.10.10.223/c/login` (bypasses OIDC redirect). Log in with Harbor admin password.
