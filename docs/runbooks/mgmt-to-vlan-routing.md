...
Devices on the management network (e.g., your Mac at 192.168.1.214) cannot reach VLAN devices (e.g., Talos node at 10.10.10.10), even with a static route pointing 10.10.0.0/16 to OPNSense WAN (192.168.1.245).

**Symptoms:**
- `ping 10.10.10.10` from Mac: timeout
- `nc -zv 10.10.10.10 6443`: "Can't assign requested address"
...
1. **Outbound NAT rewrites return traffic**: Automatic outbound NAT translates ALL traffic from PROD exiting the WAN interface, including traffic to 192.168.1.0/24. Return packets from 10.10.10.10 to 192.168.1.214 get source-rewritten to 192.168.1.245, breaking end-to-end connectivity.
...
sudo route add -net 10.11.0.0/16 192.168.1.245
```
...
```
10.10.0.0/16 via 192.168.1.245
10.11.0.0/16 via 192.168.1.245
```
...
nc -zv 10.10.10.10 6443   # Talos API (should connect)

# From OPNSense shell (Diagnostics > Shell)
ping -S 192.168.1.245 192.168.1.214   # Ping Mac from WAN IP
```
...
Mac (192.168.1.214)
  |
  | dst: 10.10.10.10 (static route via 192.168.1.245)
  v
Home Router (192.168.1.1)
  |
  | forwards to 192.168.1.245 (L2, same subnet)
  v
OPNSense WAN (192.168.1.245)
  |
...
OPNSense PROD (10.10.10.1)
  |
  | Delivers to 10.10.10.10
  v
Talos Node (10.10.10.10)
  |
