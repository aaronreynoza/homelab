...
Devices on the management network (e.g., your Mac at <WORKSTATION_IP>) cannot reach VLAN devices (e.g., Talos node at 10.10.10.10), even with a static route pointing 10.10.0.0/16 to OPNSense WAN (<OPNSENSE_WAN_IP>).
...
1. **Outbound NAT rewrites return traffic**: Automatic outbound NAT translates ALL traffic from PROD exiting the WAN interface, including traffic to 192.168.1.0/24. Return packets from 10.10.10.10 to <WORKSTATION_IP> get source-rewritten to <OPNSENSE_WAN_IP>, breaking end-to-end connectivity.
...
Mac (<WORKSTATION_IP>)
  |
  | dst: 10.10.10.10 (static route via <OPNSENSE_WAN_IP>)
  v
